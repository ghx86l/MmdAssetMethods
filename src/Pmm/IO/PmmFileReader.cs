using MmdAssetMethods.Common;
using MmdAssetMethods.Extension;
using MmdAssetMethods.Pmm.ElementState;
using MmdAssetMethods.Pmm.Frame;

namespace MmdAssetMethods.Pmm.IO;
...
public static class PmmFileReader
{
    ...
    private static System.Text.Encoding PmmEncoding { get; set; } = Encoding.Get(932);
    ...
    {
        Read(filePath, pmm, Encoding.Get(932));
    }

    public static void Read(string filePath, PolygonMovieMaker pmm, System.Text.Encoding encoding)
    {
        try
        {
            using FileStream file = new(filePath, FileMode.Open);
            using BinaryReader reader = new(file, encoding);
            Read(reader, pmm, encoding);
        }
        catch (Exception)
        {
            throw;
    ...
    {
        Read(reader, pmm, Encoding.Get(932));
    }

    public static void Read(BinaryReader reader, PolygonMovieMaker pmm, System.Text.Encoding encoding)
    {
        try
        {
            PmmEncoding = encoding;
    ...
    private static void ReadHeader(BinaryReader reader, PolygonMovieMaker pmm)
    {
        ...
        pmm.Version = reader.ReadString(30, PmmEncoding, '\0');
    ...
    private static void ReadModels(BinaryReader reader, PolygonMovieMaker pmm)
    {
        ...
        // モデル情報を保持するリスト（読み込み順を記録）
        var modelInfoList = new List<(PmmModel Model, byte ModelId, byte RenderOrder, byte CalculateOrder, int ReadIndex)>();
        ...
        for (int i = 0; i < modelCount; i++)
        {
            ...
            // 読み込み順を記録
            modelInfoList.Add((model, modelId, renderOrder, calculateOrder, i));
        }
        ...
        // 重要：描画順/計算順でソートし、同じ値なら読み込み順を保持
        var sortedByRenderOrder = modelInfoList
            .OrderBy(m => m.RenderOrder)
            .ThenBy(m => m.ReadIndex)
            .ToList();

        var sortedByCalculateOrder = modelInfoList
            .OrderBy(m => m.CalculateOrder)
            .ThenBy(m => m.ReadIndex)
            .ToList();

        // 順序リストを直接再構築
        pmm.ModelRenderOrder.Clear();
        pmm.ModelRenderOrder.AddRange(sortedByRenderOrder.Select(m => m.Model));

        pmm.ModelCalculateOrder.Clear();
        pmm.ModelCalculateOrder.AddRange(sortedByCalculateOrder.Select(m => m.Model));
    ...
    private static void ReadAccessories(BinaryReader reader, PolygonMovieMaker pmm)
    {
        ...
        // アクセサリ情報を保持するリスト（読み込み順を記録）
        var accessoryInfoList = new List<(PmmAccessory Accessory, byte RenderOrder, int ReadIndex)>();
        ...
        for (int i = 0; i < accessoryCount; i++)
        {
            ...
            // 読み込み順を記録
            accessoryInfoList.Add((accessory, renderOrder, i));
        }
        ...
        // 描画順でソートし、同じ値なら読み込み順を保持（安定）
        var sortedByRenderOrder = accessoryInfoList
            .OrderBy(a => a.RenderOrder)
            .ThenBy(a => a.ReadIndex)
            .ToList();

        // 順序リストを直接再構築（SetRenderOrderは使わない）
        pmm.AccessoryRenderOrder.Clear();
        pmm.AccessoryRenderOrder.AddRange(sortedByRenderOrder.Select(a => a.Accessory));
    }
    ...
    private static void ReadMedia(BinaryReader reader, PolygonMovieMaker pmm)
    {
        ...
        var bgmPath = reader.ReadString(256, PmmEncoding, '\0');
        ...
        var bgvPath = reader.ReadString(256, PmmEncoding, '\0');
        ...
        var bgiPath = reader.ReadString(256, PmmEncoding, '\0');
    ...
    private static void ReadFollowingSettings(BinaryReader reader, PolygonMovieMaker pmm)
    {
        ...
        pmm.Camera.Current.FollowingModel = currentCameraFollowingModelIndex >= 0 && ModelIdMap.TryGetValue(currentCameraFollowingModelIndex, out var currentCameraFollowingModel)
            ? currentCameraFollowingModel
            : null;
        pmm.Camera.Current.FollowingBone = pmm.Camera.Current.FollowingModel != null && currentCameraFollowingBoneIndex >= 0 && currentCameraFollowingBoneIndex < pmm.Camera.Current.FollowingModel.Bones.Count ? pmm.Camera.Current.FollowingModel.Bones[currentCameraFollowingBoneIndex] : null;
    ...
    private static (PmmModel Model, byte modelId, byte RenderOrder, byte CalculateOrder) ReadModel(BinaryReader reader)
    {
        ...
        model.Path = reader.ReadString(256, PmmEncoding, '\0');
    ...
    private static (PmmAccessory Accessory, byte RenderOrder) ReadAccessory(BinaryReader reader, PolygonMovieMaker pmm)
    {
        ...
        _ = reader.ReadByte();
        var name = reader.ReadString(100, PmmEncoding, '\0');
        var path = reader.ReadString(256, PmmEncoding, '\0');
    ...
    private static PmmCameraFrame ReadCameraFrame(BinaryReader reader, PolygonMovieMaker pmm, bool isInitial = false)
    {
        ...
        frame.FollowingModel = followingModelIndex >= 0 && ModelIdMap.TryGetValue(followingModelIndex, out var followingModel)
            ? followingModel
            : null;
        frame.FollowingBone = frame.FollowingModel != null && followingBoneIndex >= 0 && followingBoneIndex < frame.FollowingModel.Bones.Count
            ? frame.FollowingModel.Bones[followingBoneIndex]
            : null;
