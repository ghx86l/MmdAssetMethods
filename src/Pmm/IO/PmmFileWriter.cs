...
public static class PmmFileWriter
{
    ...
    {
        Write(filePath, pmm, Encoding.Get(932));
    }

    public static void Write(string filePath, PolygonMovieMaker pmm, System.Text.Encoding encoding)
    {
        try
        {
            using FileStream file = new(filePath, FileMode.Create);
            using BinaryWriter writer = new(file, encoding);
            Write(writer, pmm, encoding);
        }
        catch (Exception)
        {
            throw;
    ...
    {
        Write(writer, pmm, Encoding.Get(932));
    }

    public static void Write(BinaryWriter writer, PolygonMovieMaker pmm, System.Text.Encoding encoding)
    {
        PmmEncoding = encoding;
        writer.Write(pmm.Version, 30, PmmEncoding);
        ...
        {
            writer.Write(acs.Name, 100, PmmEncoding);
        }
        ...
        writer.Write(pmm.BackGround.Image ?? "", 256, PmmEncoding);
        ...
        var currentCameraFollowingModelId = pmm.Camera.Current.FollowingModel is null ? -1 : pmm.Models.Select((m, i) => (m, i)).First(p => ReferenceEquals(p.m, pmm.Camera.Current.FollowingModel)).i;
        writer.Write(currentCameraFollowingModelId);
    ...
    private static void WriteAccessory(BinaryWriter writer, PmmAccessory accessory, int index, PolygonMovieMaker pmm)
    {
        ...
        writer.Write(accessory.Name, 100, PmmEncoding);
        writer.Write(accessory.Path, 256, PmmEncoding);
    ...
    private static void WriteCamera(BinaryWriter writer, PmmCamera camera, PolygonMovieMaker pmm)
    {
        WriteFrames(
            ...
            (writer, f) =>
            {
                ...
                var frameFollowingModelId = frame.FollowingModel is null ? -1 : pmm.Models.Select((m, i) => (m, i)).First(p => ReferenceEquals(p.m, frame.FollowingModel)).i;
                writer.Write(frameFollowingModelId);
    ...
    private static void WriteModel(BinaryWriter writer, PolygonMovieMaker pmm, PmmModel model, int index)
    {
        ...
        writer.Write(model.Path, 256, PmmEncoding);
